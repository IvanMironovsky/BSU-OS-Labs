...
size_t Controller::safeUnsignedItegerInput(std::istream& in, std::ostream& out, 
	const std::string& inputPrompt, const std::string& inputFailedPrompt)
{
...
double Controller::safePositiveDoubleInput(std::istream& in, std::ostream& out, 
	const std::string& inputPrompt, const std::string& inputFailedPrompt)
{
	constexpr auto maxStreamSize = std::numeric_limits<std::streamsize>::max();
	double tempAns = -1;
...
bool Controller::printBinaryFile(const std::string& binFile, std::ostream& out)
{
	...
	for (size_t i = 0; i < numberOfEntries; ++i) {
		...
		out << std::setw(Controller::numWidth) << employee.num
...
bool Controller::createReportFile(const std::string& binFile, const std::string& reportFile, const double payment)
{
	std::ifstream fin(binFile.c_str(), std::ios::binary);
	...
	fout << std::setw(Controller::numWidth) << "id"
	...
	for(size_t i = 0; i < numberOfEntries; ++i) {
		Employee employee;
		fin.read(reinterpret_cast<char*>(&employee), sizeof(Employee));
		fout << std::setw(Controller::numWidth) << employee.num
			<< std::setw(Controller::nameWidth) << employee.name
			<< std::setw(Controller::hoursWidth) << employee.hours
